...
const DataCheck = require("../cjs/TumAmount").Factory().DataCheck

describe("test DataCheck", function () {
  xit("throw error if wallet factory doesn't have property KeyPair", function () {
    const Factory = require("../cjs/TumAmount").Factory().DataCheck
  ...
  describe("test isAmount", function () {
    ...
    it("return true when the chain is bizain and the amount is valid", function () {
      ...
      const DataCheck =
        require("../cjs/TumAmount").Factory(WalletFactory).DataCheck
      let valid = DataCheck.isAmount({
        currency: "TS1",
