...
makeEncodeDecodeTest(
  api.encodeAccountID,
  api.decodeAccountID,
  "jJjRMgiRgjU6hDF4pgu5DXQdWyPbY35EjN",
...
test("isValidClassicAddress - secp256k1 address valid", function() {
  expect(api.isValidClassicAddress("jpmKEm2sUevfpFjS7QHdT8Sx7ZGoEXTJAz")).toBe(
    true
  )
})

test("isValidClassicAddress - ed25519 address valid", function() {
  expect(api.isValidClassicAddress("jfqiMxoT228vp3dMrXKnJXo6V9iYEx94pt")).toBe(
    true
  )
})

test("isValidClassicAddress - invalid", function() {
  expect(api.isValidClassicAddress("jfqiMxoT228vp3dMrXKnJXo6V9iYEx94pz")).toBe(
    false
  )
  expect(api.isValidClassicAddress("jpmKEm2sUevfpFjS7QHdT8Sx7ZGoEXTJAa")).toBe(
    false
  )
})
...
describe("encodeSeed", function() {
  it("encodes a secp256k1 seed", function() {
    ...
    expect(result).toBe("sn259jEFXjQjWyx3Q7XneWcwV6dfL")
  })
  ...
  it("encodes high ed25519 seed", function() {
    ...
    expect(result).toBe("sEdV19BLfeQeKdEXyYA4NhrPJe6XBfG")
  })
...
describe("decodeSeed", function() {
  ...
  it("can decode a secp256k1 seed", function() {
    const decoded = api.decodeSeed("sn259jEFXjQjWyx3Q7XneWcwV6dfL")
