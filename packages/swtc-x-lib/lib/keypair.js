 ...
 * 
*/
KeyPair.prototype.verify = function (hash, sig) {
    /** specific functions for ecdsa publicKey. */
    if (!this._pubkey) {
        console.log("public key verifying:", hash);
        return this._pubkey.verify(hash, sig, 0);
    }
    else {
        console.log("PUBKEY not found!");
        this._pubkey = this._pub();
        if (!this._pubkey) {
            console.log("PUBKEY is empty!");
        }
        else {
            console.log("Regenerate PUBKEY!", this.to_hex_pub())
        }
        return this._pubkey.verify(hash, sig, 0);
    }
};
