...
async function main() {
  ...
  try {
    ...
    tx = remote.buildOfferCreateTx({
      ...
      taker_pays: remote.makeAmount(0.1, "CNY", DATA.issuer)
    })
    tx.setSecret(DATA.secret)
    tx.sign((error, blob) => {
      if (error) {
    ...
    tx = remote.buildRelationTx({
      ...
      account: DATA.address,
      limit: remote.makeAmount(Math.floor(Math.random() * 10), "CNY", DATA.issuer)
    })
    // tx.setSequence(sequence)
    tx.setSecret(DATA.secret)
    tx.sign((error, blob) => {
...
async function main() {
  try {
    ...
    result = await remote.postBlob({
      blob: ""
    })
    console.log(result)
  } catch (error) {
    console.log(error)
