...
$ tns run android
```
7. Vue的默认页面内容文件 app/components/Home.vue 如下
```vue
<template>
    <Page class="page">
...
<template>
    <Page class="page">
        ...
        <GridLayout rows="auto,*,auto">
            ...
            <StackLayout row="2">
                <Button class="btn btn-primary full" horizontalAlignment="center">价格</Button>
...
<script>
    ...
    export default {
        ...
        computed: {
            wallet_text() { return JSON.stringify(this.wallet, '', 2) },
            ledger_text() { return JSON.stringify(this.ledger, '', 2) },
            price_text() { return this.price.join('\n') }
        },
        created() {
        ...
        async mounted () {
            ...
            // 十秒钟一个钱包
            setInterval( () => { this.wallet = Wallet.generate(); console.log(this.wallet)}, 10000)
            // 十秒钟查价格
            setInterval( () => {
                remote.requestOrderBook(swt_vs_cny).submit( (error, orderbooks) => {
                    if (error) {
                        ...
                        orderbooks.offers.map( offer => {
                            ...
                            this.price.unshift(`价格: ${price}\t挂单量: ${quantity}\t${offer.Account}`)
                            this.price.splice(5)
                        })
                    }
                })}, 10000)
